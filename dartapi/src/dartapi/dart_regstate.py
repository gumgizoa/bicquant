...
from dartapi.dart_catalog import REGSTATE_CATALOG
...
def regstate(
    ...
    Args:
        ...
        report_type: 신고서 유형(API 엔드포인트 이름). 사용 가능한 값은 dart_catalog.REGSTATE_CATALOG 참조.
                     예: "estkRs", "bdRs", "mgRs"
        start: 시작일. 검색시작 접수일자(YYYYMMDD 형식 문자열 또는 datetime 객체). 선택 파라미터입니다.
        end: 종료일. 검색종료 접수일자(YYYYMMDD 형식 문자열 또는 datetime 객체). 선택 파라미터입니다.

    Returns:
        dict: 증권신고서 정보를 담은 딕셔너리. 다음 공통 구조를 가집니다:
            - status (str): 에러 및 정보 코드
            - message (str): 에러 및 정보 메시지
            - group (list): 그룹별 정보 배열. 필드는 report_type에 따라 다름.
                            dart_catalog.REGSTATE_CATALOG[report_type]["fields"]에서 필드별 상세 설명 확인 가능.

    Raises:
        ValueError: report_type이 REGSTATE_CATALOG에 없는 경우.
    """
    if report_type not in REGSTATE_CATALOG:
        valid = sorted(REGSTATE_CATALOG.keys())
        raise ValueError(f"Unknown report_type: {report_type!r}. Valid values: {valid}")
    return _call_regstate_api(api_key, corp_code, report_type, start, end)
