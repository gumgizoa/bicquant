...
from dartapi.dart_catalog import EVENT_RETURN_FIELDS_CATALOG
...
def event(
    ...
    Args:
        ...
        event_type: 이벤트 유형(API 엔드포인트 이름). 사용 가능한 값은 dart_catalog.EVENT_RETURN_FIELDS_CATALOG 참조.
    ...
    """
    if event_type not in EVENT_RETURN_FIELDS_CATALOG:
        valid = sorted(EVENT_RETURN_FIELDS_CATALOG.keys())
        raise ValueError(f"Unknown event_type: {event_type!r}. Valid values: {valid}")
    return _call_event_api(client, corp_code, event_type, start, end)
