"""해외 거시 지표 블록 포맷 (미 국채 10년물 + 미국 M2) — bot 전용 feature.

순수 함수만 담는다. FRED 데이터 페치와 텔레그램 발송은 ``bot.main``에 있다.
여기서 만든 블록은 시장 요약(``market_report``) 메시지의 마지막 블록으로 이어 붙는다
(``bot.main._send_market_report``). 이격도·ADR·신용공여 잔고와 마찬가지로 자기 헤더를 갖는다.
